...
The contract evaluates `decide/2` or `decide/3` depending on `motivated`, and records the resulting verdict (and optional motivation) as a durable decision record.

| parameter                   | description                                                                                                                                                                                                                                                |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `record_decision`           | _(Required.) _ **object**.                                                                                                                                                                                                                                 |
| `record_decision.case`      | _(Required.) _ **string**. The decision context.                                                                                                                                                                                                           |
| `record_decision.motivated` | **boolean\|null**. Whether to request a motivated decision (defaults to `false`).<br /><br />- If `false`, the contract calls `decide/2` and records only the verdict. - If `true`, the contract calls `decide/3` and records both verdict and motivation. |
...
The `case` parameter is a Prolog dict term string (typically `ctx{...}`) representing the decision context. This is passed as the `Case` argument to `decide/2` or `decide/3`.
...
| `decide.motivated` | **boolean\|null**. Whether to request a motivated decision (defaults to `false`).<br /><br />- If `false`, the contract calls `decide/2` and returns only the verdict. - If `true`, the contract calls `decide/3` and returns both verdict and motivation. |
...
_Rendered by [Fadroma](https://fadroma.tech) ([@fadroma/schema 1.1.0](https://www.npmjs.com/package/@fadroma/schema)) from `axone-gov.json` (`7187c94a387e2439`)_
